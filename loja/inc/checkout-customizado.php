...
function loja_custom_checkout($fields) {
...
add_filter('woocommerce_checkout_fields', 'loja_custom_checkout');
...
// Adiciona Campo
function loja_custom_checkout_field($checkout) {
...
add_action('woocommerce_after_order_notes', 'loja_custom_checkout_field');

// Validar Campo
function loja_custom_checkout_field_process() {
  if(!$_POST['mensagem_personalizada']) {
    wc_add_notice('Por favor, escreva uma mensagem personalizada.', 'error');
  }
}
add_action('woocommerce_checkout_process', 'loja_custom_checkout_field_process');

// Adicionar ao Banco de Dados
function loja_custom_checkout_field_update($order_id) {
...
add_action('woocommerce_checkout_update_order_meta', 'loja_custom_checkout_field_update');
